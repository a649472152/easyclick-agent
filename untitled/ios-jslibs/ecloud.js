...
ECloudWrapper.prototype.log = function (msg) {
    ...
    ecImporter.cloudLog(msg, JSON.stringify(s));
}

/**
 * 云控设置
 * 适合版本 EC iOS 脱机版本 2.2.0+
 * @param map
 * cloudUrls: 云控的地址
 * cloudDeviceNo: 本机器的编号
 * connectType: 链接方式 1 长连接 2 短链接
 * heartTime: 心跳间隔，单位是秒
 * @returns {boolean} true 代表成功 false 代表失败
 */
ECloudWrapper.prototype.setCloudConfig = function (map) {
    return ecloudWrapper.setCloudConfig(JSON.stringify(map));
}
 ...
 * 获取云控的URL地址
 * 适合版本 EC iOS 脱机版本  2.2.0+
 * @return {null|string} 云控的URL地址或者null
 */
 ...
 *      columnSize: 字段长度<br/>
 * @return {null|string} JSON字符串，自行转换为JSON对象
 ...
 * key定义：
 * cacheKey = 缓存key
 * @return {null|string} 服务端返回的JSON字符串，解析result节点即可
 */
...
ECloudWrapper.prototype.incrementCache = function (map) {
    ...
    let r = ecloudWrapper.incrementCache(JSON.stringify(map));
    if (r == "" || r == undefined || r == null) {
        return 0;
    }
    try {
        return parseInt(JSON.parse(r)["result"])
    } catch (e) {
    }
    return 0
};
...
ECloudWrapper.prototype.decrementCache = function (map) {
    ...
    let r = ecloudWrapper.decrementCache(JSON.stringify(map));
    if (r == "" || r == undefined || r == null) {
        return 0;
    }
    try {
...
ECloudWrapper.prototype.getCacheExpire = function (map) {
    ...
    let r = ecloudWrapper.getCacheExpire(JSON.stringify(map));
    if (r == "" || r == undefined || r == null) {
        return 0;
    }
    try {
