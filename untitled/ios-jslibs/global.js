 ...
 */
function appLaunch(bundleId, ignoreState) {
    return agentEvent.appLaunch(bundleId, ignoreState);
}
 ...
 * @return {boolean} true 成功，false 失败
 */
function appKillByBundleId(bundleId, ignoreState) {
    return agentEvent.appKillByBundleId(bundleId, ignoreState);
}
...
/**
 * 多点触摸<br/>
 * 触摸参数: action :一般情况下 按下为0，弹起为1，移动为2,3 暂停 <br/>
 * x: X坐标<br/>
 * y: Y坐标<br/>
...
function getMyBundleId() {
    return globalWrapper.getMyBundleId();
}
...
function getMyAppName() {
    return globalWrapper.getMyAppName();
}


function generateRandomString(length) {
    var result = '';
    var characters = '0123456789abcdefghjklmnopqrstuvwxyz'; // 可以根据需要修改字符集
